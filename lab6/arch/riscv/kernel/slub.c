...
void slub_init()
{
	...
	for (int i = 0; i < NR_PARTIAL; i++)
	{
		...
		printk("[S] Create New cache: %s size: %x, align: 8\n", slub_allocator[i]->name, slub_allocator[i]->size);
	}
	return;
}
	...
	kmem_cache_alloc(struct kmem_cache *cache)
{
	...
	printk("[S] kmem_cache_alloc: name: %s  addr: %lx, partial_obj_count: %d\n", cache->name, object, (ADDR_TO_PAGE(object)->header)->count);
	return object;
}
